...
// entrada:
//lendo tamanho da sequencia 1
	cin >> t1;
//entrando com a sequencia 1
	for(int i = 0; i < t1; i++)
		cin >> s1[i];
//lendo tamanho da sequencia 2
	cin >> t2;
//entrando com a sequencia 2
	for(int i = 0; i < t2; i++)
		cin >> s2[i];
...
// caso geral
	for(int i = t1 - 1; i >= 0; i--) //t1 ja foi processado no caso base
		for(int j = t2 - 1; j >= 0; j--)
		{
			...
			else //Amalisa qual foi a maior substring encontrada até agr, usando o "caminho"
			{
				if(pd[i + 1][j] > pd[i][j + 1]) // passo para proximo valor em 'i' -> caminho = 2;
				{
					pd[i][j] = pd[i + 1][j];
					caminho[i][j] = 2;
				}
				else // passo para proximo valor em 'j' -> caminho = 1;
				{
					pd[i][j] = pd[i][j + 1];
					caminho[i][j] = 1;
				}
