...
int main()
{
	ponto v[100]; //vetor recebe pontos
	int x; //variavel recebe quantidade de pontos
	list <ponto> pilha; //pilha para receber retorno do algoritimo

	//lendo numero de pontos
	cin >> x;

	//registrando pontos na struct
	for (int i = 0; i < x; i++)
		cin >> v[i].x >> v[i].y;

	//utilizando Graham scan para verificar numero de lados
	pilha = Graham_scan(v, x);

	//contando quantidade de lados
	int t = 0;
	while(!pilha.empty())
	{
		t = t + 1; //increment
